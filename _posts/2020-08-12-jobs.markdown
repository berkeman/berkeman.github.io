---
layout: post
title:  "Let the Job Object do the Work!"
...
of functions that makes common tasks straightforward to write and
comprehend.  This post gives an overview of the job object's
possibilities.
...
Datasets are created using `job.datasetwriter()`, here's a sketch:
...
The Accelerator has built in support for storing data in Python's
pickle format or in JSON format.  Not because it is necessary, but
because it simplifies and streamlines the coding experience.
...
and retreive complex datatypes between jobs and build scripts whithout
having to consider formatting or parsing!*

The JSON-equivalents called `job.json_save()` and `job.json_load()`.
(The Accelerator typically uses JSON format internally for meta data
files and pickle format for everything else.)
...
### So, How does the Job Know About what Files that have been Created?
...
generated by some other method, files can be registered manually, like
this:
...
can be put.  Actually, files should not be stored there, because that
would break the link between the file and the job that created the
file ("Where does this file come from again?").  Instead, we use soft
links, that *point* to files in jobs.  The difference between a file
...
is to use the helper function `job.link_result()`, like this:
...
from one job to another.  There are several ways to do this:
 ...
 - We pass the job object representing an existing job with files to a
   new job.  In this case, the new job then knows which job that
   created and holds the files, but it does not know the name of the
   files.
 ...
 - We pass the full pathname of the file to the new job as a
   parameter.  In this case, the file could be read directly by the
   new job, but the information about which job that generated the
   file is gone.  *This breaks transparency and should be avoided.*
...
well as jobname from the input parameter, and it also handles
**parallel file sets** for passing data between parallel processing
jobs, see manual for more information.



---

## 6. Miscellaneous Functions, Available for Existing Jobs
...
## 7.  Miscellaneous Functions, Available for Running Jobs
...
@@@ ta bort result_directory i CurrentJob



---

## 8. Miscellaneous Functions, Common for both Existing and Running Jobs
